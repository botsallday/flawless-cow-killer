package scripts.BADFlawlessCowKiller.api.conditions;
...
public class BADConditions {
	...
	public static Condition objectIsNear(String object_name) {
		return new Condition() {
			@Override
			public boolean active() {
                General.sleep(100, 200);
                return Objects.find(20, object_name).length > 0; //until we are teleported to the mine
			}
		};
	}
	
	public static Condition bankScreenIsClosed() {
		return new Condition() {
			@Override
			public boolean active() {
				General.sleep(100);
	...
	public static Condition hasItemEquipped(String item_name) {
		return new Condition() {
			@Override
			public boolean active() {
				General.sleep(100);
				if (Equipment.isEquipped(Filters.Items.nameContains(item_name))) {
					return true;
				};
	...
	public static Condition objectIsntNear(String object_name) {
		return new Condition() {
			@Override
			public boolean active() {
                General.sleep(100, 200);
